...
public abstract partial class SharedVisualBodySystem
{
    ...
    private void ApplyAppearanceTo(Entity<VisualBodyComponent?> ent, HumanoidCharacterAppearance appearance, Sex sex, float height)
    {
        if (!Resolve(ent, ref ent.Comp))
            return;
    ...
    {
        ApplyAppearanceTo(ent, profile.Appearance, profile.Sex, profile.CDHeight);
    }
